...
export async function GET() {
  const integrations = [
    ...
    { name: 'Filesystem MCP', status: 'not-configured', tools: 0, icon: 'folder' },
    { name: 'Brave Search MCP', status: 'not-configured', tools: 0, icon: 'search' },
  ];
  return NextResponse.json({ integrations, vpsBridgeUrl: 'https://187.127.250.171:8400' });
}
