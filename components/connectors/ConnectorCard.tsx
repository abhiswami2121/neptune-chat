...
const STATUS_CONFIG: Record<string, { color: string; bg: string }> = {
  Connected: { color: "text-emerald-500", bg: "bg-emerald-500/10 ring-emerald-500/20" },
  "Not Configured": { color: "text-yellow-500", bg: "bg-yellow-500/10 ring-yellow-500/20" },
  Error: { color: "text-red-500", bg: "bg-red-500/10 ring-red-500/20" },
};

export function ConnectorCard({ manifest, status, onClick, index = 0 }: ConnectorCardProps) {
  const statusLabel = status.connected ? "Connected" : status.message?.includes("Missing") ? "Not Configured" : status.message || "Unknown";
