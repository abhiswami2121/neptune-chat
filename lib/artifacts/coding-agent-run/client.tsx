...
export function CodingAgentRunArtifact({ runId, status, prompt, streamUrl }: CodingAgentRunProps) {
  const [steps, setSteps] = React.useState<Array<{ id: string; title: string; status: string }>>([]);
  const [terminal, setTerminal] = React.useState('');
  const [files, setFiles] = React.useState<Array<{ path: string; content: string }>>([]);
  ...
  React.useEffect(() => {
    ...
    evtSource.addEventListener('step', (e) => {
      ...
      setSteps(prev => [...prev, { id: data.id, title: data.title, status: data.status }]);
    });
    ...
    <div className="grid grid-cols-12 gap-4 h-full p-4">
      <div className="col-span-3 border rounded-lg p-3 overflow-y-auto">
        ...
        {steps.map(step => (
          <div key={step.id} className="flex items-center gap-2 text-xs py-1">
            <span className={step.status === 'completed' ? 'text-green-500' : step.status === 'running' ? 'text-blue-500 animate-pulse' : 'text-gray-400'}>
              {step.status === 'completed' ? '✓' : step.status === 'running' ? '⏳' : '○'}
            </span>
            <span>{step.title}</span>
          </div>
        ))}
        {steps.length === 0 && <p className="text-xs text-gray-400">Waiting for agent to start...</p>}
      </div>
          ...
          <div className="space-y-2">
            {files.map(f => (
              <div key={f.path} className="border rounded p-2">
                ...
                <pre className="text-xs mt-1 bg-gray-50 p-2 rounded overflow-x-auto max-h-40">{f.content}</pre>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-xs text-gray-400">Files will appear as the agent works...</p>
        )}
        {prompt && (
      ...
      <div className="col-span-3 flex flex-col gap-3">
        <div className="border rounded-lg p-3 flex-1 overflow-y-auto bg-black text-green-400 font-mono text-xs">
          ...
          <pre className="whitespace-pre-wrap">{terminal || 'Awaiting output...'}</pre>
        </div>
        {previewUrl && (
          <div className="border rounded-lg p-3">
            <h3 className="font-semibold text-sm mb-2">Preview</h3>
            <iframe src={previewUrl} className="w-full h-40 border rounded" title="Preview" />
          </div>
        )}
      </div>
    </div>
