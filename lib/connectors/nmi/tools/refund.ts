...
export const refundTransaction = tool({
  description: "Process a refund for a settled NMI transaction. Proxied through VPS bridge.",
  inputSchema: z.object({
    transactionId: z.string().describe("NMI transaction ID to refund"),
    amount: z.number().positive().optional().describe("Amount to refund (defaults to full amount)"),
  }),
