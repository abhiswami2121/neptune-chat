...
export const queryTransactions = tool({
  description: "Query NMI transactions in a date range. Proxied through VPS bridge (NMI keys stay on VPS).",
  inputSchema: z.object({
    startDate: z.string().describe("Start date YYYYMMDD"),
    endDate: z.string().describe("End date YYYYMMDD"),
    condition: z.enum(["complete", "failed", "pending"]).optional().describe("Filter by transaction condition"),
