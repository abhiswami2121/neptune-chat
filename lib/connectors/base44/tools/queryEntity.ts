...
export const queryEntity = tool({
  description: "Query a Base44 entity with MongoDB-style filter. Returns matching records.",
  inputSchema: z.object({
    ...
    sort: z.string().optional().describe("Sort direction, e.g. '-created_date'"),
  ...
  execute: async ({ entity, filter, sort, limit }) => {
    try {
      ...
      return { entity, count: Array.isArray(results) ? results.length : 0, results };
    } catch (err) {
      return { error: `Base44 query failed: ${err instanceof Error ? err.message : "Unknown"}` };
    }
  },
});
