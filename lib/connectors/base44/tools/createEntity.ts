...
export const createEntity = tool({
  description: "Create a new Base44 entity record. Returns the created record with ID.",
  ...
  execute: async ({ entity, data }) => {
    try {
      const record = await base44Service.entities[entity].create(data as Record<string, unknown>);
      return { created: true, entity, record };
    } catch (err) {
      return { error: `Base44 create failed: ${err instanceof Error ? err.message : "Unknown"}` };
    }
  },
});
