...
function inferColumns(results: Record<string, unknown>[]): string[] {
  ...
  const priority = ["name", "email", "stage", "mrr", "amount", "status", "created_date", "id"];
...
export default function EntityTable({ output }: { output: EntityOutput }) {
  ...
  return (
    <Card className="border-t-2 overflow-hidden" style={{ borderTopColor: "#7C3AED" }}>
      <div className="flex items-center gap-2 px-4 py-3 bg-[#7C3AED]/5 border-b">
        <DatabaseIcon className="w-4 h-4 text-[#7C3AED]" />
        <span className="font-medium text-sm text-[#7C3AED]">{output?.entity ?? "Entity"}</span>
        <Badge variant="secondary" className="ml-auto text-[10px]">{output?.count ?? results.length} records</Badge>
      </div>
      <Table>
        <TableHeader>
          <TableRow>
            {columns.map((col) => (
              <TableHead key={col} className="text-[10px] first:pl-4">{col}</TableHead>
            ))}
          </TableRow>
        </TableHeader>
        <TableBody>
          {results.map((row, i) => (
            <TableRow key={i}>
              {columns.map((col) => (
                <TableCell key={col} className="text-[10px] first:pl-4 font-mono">{formatCell(row[col])}</TableCell>
              ))}
            </TableRow>
          ))}
        </TableBody>
