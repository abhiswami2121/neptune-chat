 ...
 */
import { ZapIcon, CheckCircleIcon, XCircleIcon, ClockIcon } from "lucide-react";
...
export default function PaymentList({ output }: { output: PaymentOutput }) {
  ...
  return (
    <Card className="border-t-2 overflow-hidden" style={{ borderTopColor: "#006FEE" }}>
      <div className="flex items-center gap-2 px-4 py-3 bg-[#006FEE]/5 border-b">
        <ZapIcon className="w-4 h-4 text-[#006FEE]" />
        <span className="font-medium text-sm text-[#006FEE]">{payments.length} payments</span>
      </div>
      <Table>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {payments.map((p, i) => {
            const st = statusMap[p.status ?? ""] ?? { color: "text-muted-foreground", label: p.status ?? "unknown" };
            return (
              <TableRow key={p.payment_id || i}>
                <TableCell className="text-[10px] font-mono">{p.payment_id?.slice(0, 16) ?? "-"}</TableCell>
                <TableCell className="text-[10px] text-right font-medium">{(p.amount ?? 0) / 100} {p.currency ?? "USD"}</TableCell>
                <TableCell>
                  <Badge variant="secondary" className={cn("gap-1 text-[9px]", st.color)}>{st.label}</Badge>
                </TableCell>
                <TableCell>
