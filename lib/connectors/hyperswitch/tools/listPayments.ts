...
export const listPayments = tool({
  description: "List recent Hyperswitch payments. Includes payment status and connector info (NMI chip).",
  inputSchema: z.object({
    limit: z.number().int().min(1).max(100).optional().default(20),
    customerId: z.string().optional().describe("Filter by customer ID"),
    status: z.enum(["succeeded", "failed", "processing", "requires_payment_method"]).optional(),
  }),
  execute: async (input) => {
