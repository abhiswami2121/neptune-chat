...
export const createPaymentLink = tool({
  description: "Create a Hyperswitch payment link for a customer. Returns a branded pay.newleaf.financial URL.",
  inputSchema: z.object({
    ...
    styleId: z.string().optional().default("newleaf-sub-signup").describe("Style ID for checkout branding"),
    description: z.string().optional().describe("Payment description shown to customer"),
  }),
  execute: async (input) => {
    ...
    try {
      ...
      const res = await fetch(`${BRIDGE_URL}/tool/hyperswitch/createPaymentLink`, {
        method: "POST",
