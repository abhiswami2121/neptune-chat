...
export const postMessage = tool({
  ...
  inputSchema: z.object({
    ...
    text: z.string().max(40000).describe("Message text (max 40,000 chars)"),
  ...
  execute: async ({ channel, text, threadTs }) => {
    ...
    try {
      const res = await slack.chat.postMessage({ channel, text, ...(threadTs ? { thread_ts: threadTs } : {}) });
      if (!res.ok) return { error: `Slack post failed: ${res.error}` };
      return { ok: true, channel: res.channel, ts: res.ts, message: res.message?.text };
    } catch (err) {
      return { error: `Slack post failed: ${err instanceof Error ? err.message : "Unknown"}` };
    }
  },
});
