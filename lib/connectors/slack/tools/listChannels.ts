...
export const listChannels = tool({
  ...
  inputSchema: z.object({
    types: z.string().optional().default("public_channel,private_channel").describe("Channel types to list"),
  ...
  execute: async ({ types, limit }) => {
    ...
    try {
      ...
      const channels = (res.channels ?? []).map((c) => ({
        ...
        memberCount: c.num_members, topic: c.topic?.value?.slice(0, 100),
      }));
      return { count: channels.length, channels };
    } catch (err) {
      return { error: `List failed: ${err instanceof Error ? err.message : "Unknown"}` };
    }
  },
});
